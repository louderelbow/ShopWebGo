...
func (con BaseController) Render(c *gin.Context, tpl string, data map[string]interface{}) {
	...
	//1、获取顶部导航
	topNavList := []model.Nav{}
	if hasTopNavList := util.CacheDb.Get("topNavList", &topNavList); !hasTopNavList {
		util.DB.Where("status=1 AND position=1").Find(&topNavList)
	...
	if hasGoodsCateList := util.CacheDb.Get("goodsCateList", &goodsCateList); !hasGoodsCateList {
		...
		util.DB.Where("pid = 0 AND status=1").Order("sort DESC").Preload("GoodsCateItems", func(db *gorm.DB) *gorm.DB {
	...
	if hasMiddleNavList := util.CacheDb.Get("middleNavList", &middleNavList); !hasMiddleNavList {
		...
		for i := 0; i < len(middleNavList); i++ {
			...
			util.DB.Where("id in ?", relationIds).Select("id,title,goods_img,price").Find(&goodsList)
