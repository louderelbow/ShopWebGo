...
func (con PassController) Captcha(c *gin.Context) {
	id, b64s, err := util.MakeCaptcha(50, 120, 2)
...
func (con PassController) Login(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "shopWeb/pass/login.html", gin.H{
		"prevPage": prevPage,
	})
}
...
func (con PassController) RegisterStep1(c *gin.Context) {
	c.HTML(http.StatusOK, "shopWeb/pass/register_step1.html", gin.H{})
}
...
func (con PassController) RegisterStep2(c *gin.Context) {
	...
	if len(userTemp) > 0 {
		c.HTML(http.StatusOK, "shopWeb/pass/register_step2.html", gin.H{
...
func (con PassController) RegisterStep3(c *gin.Context) {
	...
	util.DB.Where("sign=?", sign).Find(&userTemp)
	if len(userTemp) > 0 {
		c.HTML(http.StatusOK, "shopWeb/pass/register_step3.html", gin.H{
			"smsCode": smsCode,
			"sign":    sign,
		})
	} else {
...
func (con PassController) SendCode(c *gin.Context) {
	...
	*/
	...
	var sendCount int64
	util.DB.Table("user_temp").Where("ip=? AND add_day=?", ip, currentDay).Count(&sendCount)
	if sendCount > 4 {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
	...
	util.DB.Where("phone = ? AND add_day=?", phone, currentDay).Find(&userTemp)
	if len(userTemp) > 0 {
		if userTemp[0].SendCount > 2 {
			...
			//3、更新发送短信的次数
			oneUserTemp := model.UserTemp{}
			util.DB.Where("id=?", userTemp[0].Id).Find(&oneUserTemp)
			oneUserTemp.SendCount = oneUserTemp.SendCount + 1
			oneUserTemp.AddTime = int(util.GetUnix())

			util.DB.Save(&oneUserTemp)
