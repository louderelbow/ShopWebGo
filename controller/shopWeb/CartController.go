...
func (con CartController) Get(c *gin.Context) {
	...
	var tpl = "shopWeb/cart/cart.html"
...
func (con CartController) AddCart(c *gin.Context) {
	...
	*/
	...
	colorId, _ := util.Int(c.Query("color_id"))
	goodsId, err := util.Int(c.Query("goods_id"))
	if err != nil {
		c.Redirect(302, "/")
	}
	...
	util.DB.Where("id=?", colorId).Find(&goodsColor)
	...
	if len(cartList) > 0 {
		...
		if model.HasCartData(cartList, currentData) {
			for i := 0; i < len(cartList); i++ {
	...
	c.Redirect(302, "/cart/successTip?goods_id="+util.String(goodsId))
}
...
func (con CartController) AddCartSuccess(c *gin.Context) {
	...
	var tpl = "shopWeb/cart/addcart_success.html"
	con.Render(c, tpl, gin.H{
		"goods": goods,
	})
}
...
func (con CartController) DelCart(c *gin.Context) {
	...
	goodsId, _ := util.Int(c.Query("goods_id"))
	goodsColor := c.Query("goods_color")
	GoodsAttr := ""
